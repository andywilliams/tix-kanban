...
// Follow up on in-progress tasks that have PRs — check for comments and conflicts
async function followUpInProgressPRs(): Promise<void> {
  try {
    const tasks = await getAllTasks();
    const inProgressWithPRs = tasks.filter(task => {
      if (task.status !== 'in-progress') return false;
      const hasPR = (task.links || []).some(l => l.type === 'pr' || l.url?.includes('/pull/'));
      return hasPR;
    });

    if (inProgressWithPRs.length === 0) {
      return;
    }

    console.log(`🔍 Checking ${inProgressWithPRs.length} in-progress task(s) with PRs...`);

    for (const task of inProgressWithPRs) {
      try {
        const prLinks = (task.links || []).filter(l => l.type === 'pr' || l.url?.includes('/pull/'));
        let hasIssues = false;
        ...
        for (const link of prLinks) {
          ...
          try {
            // Check PR state, comments, and merge conflicts
            const { execSync } = await import('child_process');
            
            // Get PR review comments (code review comments)
            const reviewComments = execSync(
              `gh api repos/${repo}/pulls/${prNumber}/comments --jq '[.[] | select(.user.login != "github-actions[bot]")] | length'`,
              { encoding: 'utf-8', stdio: 'pipe', timeout: 15000 }
            ).trim();

            // Get PR issue comments (general discussion)  
            const issueComments = execSync(
              `gh api repos/${repo}/issues/${prNumber}/comments --jq '[.[] | select(.user.type != "Bot")] | length'`,
              { encoding: 'utf-8', stdio: 'pipe', timeout: 15000 }
            ).trim();

            // Check mergeable state
            const mergeState = execSync(
              `gh pr view ${prNumber} --repo ${repo} --json mergeable,state --jq '.mergeable + " " + .state'`,
              { encoding: 'utf-8', stdio: 'pipe', timeout: 15000 }
            ).trim();

            const [mergeable, state] = mergeState.split(' ');

            // Skip if already merged or closed
            if (state === 'MERGED' || state === 'CLOSED') {
              console.log(`✅ PR #${prNumber} (${repo}) is ${state.toLowerCase()} — moving task to review`);
              await updateTask(task.id, { status: 'review' });
              continue;
            }

            const totalComments = parseInt(reviewComments || '0') + parseInt(issueComments || '0');
            
            if (mergeable === 'CONFLICTING') {
              hasIssues = true;
              issueDetails.push(`PR #${prNumber} has merge conflicts`);
            }

            if (totalComments > 0) {
              // Check if there are comments newer than the last worker comment
              const latestNonBotComment = execSync(
                `gh api repos/${repo}/pulls/${prNumber}/comments --jq '[.[] | select(.user.login != "github-actions[bot]") | select(.user.login | contains("bot") | not)] | last | .created_at // empty'`,
                { encoding: 'utf-8', stdio: 'pipe', timeout: 15000 }
              ).trim();

              const latestIssueComment = execSync(
                `gh api repos/${repo}/issues/${prNumber}/comments --jq '[.[] | select(.user.type != "Bot")] | last | .created_at // empty'`,
                { encoding: 'utf-8', stdio: 'pipe', timeout: 15000 }
              ).trim();

              if (latestNonBotComment || latestIssueComment) {
                hasIssues = true;
                issueDetails.push(`PR #${prNumber} has unresolved comments`);
              }
            }

            if (!hasIssues) {
              // PR is clean — no conflicts, no outstanding comments
              console.log(`✅ PR #${prNumber} (${repo}) is clean — moving task to review`);
              await updateTask(task.id, { status: 'review' });
            }
          } catch (error) {
            console.warn(`Failed to check PR status for ${link.url}:`, error);
          }
        }

        if (hasIssues) {
          console.log(`⚠️  Task "${task.title}" has PR issues: ${issueDetails.join(', ')}`);
          
          // Load persona and spawn AI to address the issues
          ...
          console.log(`🔧 Spawning AI to address PR issues for: ${task.title}`);
          const aiResult = await spawnAISession(fullTask, persona);
          
          // Update persona memory
          await updatePersonaMemoryAfterTask(
            persona.id,
            `PR follow-up: ${fullTask.title}`,
            `Address PR comments and/or merge conflicts: ${issueDetails.join(', ')}`,
            aiResult.output,
            aiResult.success
          );
          ...
          const followUpComment: Comment = {
            id: Math.random().toString(36).substr(2, 9),
            taskId: fullTask.id,
            body: `**PR Follow-up:** ${issueDetails.join(', ')}\n\n${aiResult.output}`,
            author: `${persona.name} (AI)`,
            createdAt: new Date(),
          };
          await updateTask(fullTask.id, {
            comments: [...existingComments, followUpComment]
          });

          console.log(`${aiResult.success ? '✅' : '❌'} PR follow-up completed for: ${task.title}`);
        }
      } catch (error) {
        console.error(`Failed to follow up on task ${task.id}:`, error);
      }
    }
  } catch (error) {
    console.error('❌ PR follow-up processing failed:', error);
  }
}
...
async function runWorker(): Promise<void> {
  ...
  try {
    ...
    // Follow up on in-progress tasks with PRs (check comments, conflicts)
    await followUpInProgressPRs();
